"""Tab 4 — diagnostics on the parallel chain.

Ports the §3-§8 sections from the PbAction toolkit:
  §3  step-by-step calculation walkthrough (every chain equation per year)
  §4  two residuals + indicators (install, refine shortfall, refine overshoot)
  §5  k/τ ridge — install-ceiling vs USGS-refine-floor crossings
  §6  φ↔η_I closed-form curves (one per stage)
  §7  feasibility-region map over (k, τ) with the current point marked
  §8  Monte Carlo (parametric, opt-in)

Every section reads from app.state.compute_state() so the canonical
(k, τ, β, γ, φ, η, stock, segments) live in one place. Editing anything
in the sidebar or Tab 2 updates this tab on the next rerun.
"""
...
from india_model.model_v4 import (
    retire_rate, FEED_HS, PARTS_HS, CRUDE_HS,
)
...
from app.state import compute_state, load_raw_csv, FIT_WINDOW, build_arr_from_user_inputs

EFF_DEFAULTS = dict(V5_ETA_DEFAULTS)


# ---------------------------------------------------------------------------
# Helpers (formatting + cached file loading is in app.state)
# ---------------------------------------------------------------------------
...
# Note: data loading + arr building lives in app.state — see compute_state().
...
def render() -> None:
    """Render the diagnostics tab — §3-§8 of the original toolkit dashboard.

    Reads the canonical state via compute_state() so every input (sidebar's
    β/γ/η, Tab 2's stock/segments/k/τ/φ) propagates here automatically.
    """
    state = compute_state()
    arr           = state["arr"]
    chain         = state["chain"]
    g_val         = state["g"]
    tau_main      = state["tau_main"]
    beta          = state["beta"]
    gamma         = state["gamma"]
    k_main        = state["k"]
    phi           = state["phi"]
    ...
    st.markdown(
        f"### Diagnostics on the parallel chain  "
        f"`(k = {k_main:.2f}, τ = {tau_main:.2f} yr, β = {beta:.2f}, γ = {gamma:.2f})`"
    )
    st.caption(
        "Sections 3-8 of the original toolkit dashboard, wired to the canonical "
        "state. Edit anything in the sidebar or Tab 2 and these sections update "
        "on the next rerun."
    )

    # ----- §3 walkthrough --------------------------------------------------
    st.divider()
    st.markdown("#### 3. Calculation walkthrough — every step from inputs to install")
    ...
    # ----- §4 residuals + indicators ---------------------------------------
    st.divider()
    st.markdown(f"#### 4. Two residuals + indicators")
    ...
    # ----- §6 φ↔η_I curves -------------------------------------------------
    st.divider()
    st.markdown("#### 6. φ ↔ η_I closed-form curves — one per stage")
    _render_phi_eta_curves(
        arr, k=k_main, g=g_val, tau=tau_main,
        gamma=gamma, beta=beta, etas=etas, phi=phi,
    )

    # ----- §7 feasibility-region map ---------------------------------------
    st.divider()
    st.markdown(
        f"#### 7. Feasibility-region map"
        f"  ·  current (k, τ) = ({k_main:.2f}, {tau_main:.2f})"
    )
    ...
    # ----- footnotes -------------------------------------------------------
    with st.expander("Reading the diagnostics"):
        st.markdown(
            "- The **two residuals are always shown separately**. There is no "
            "blended fit score by design.\n"
            "- Refine is a one-sided FLOOR (USGS ≤ REFINE_SEC_F); overshoot is "
            "the implied unrecorded refined Pb.\n"
            "- The **k/τ ridge** shows where install-ceiling and refine-floor "
            "cross. Where they overlap, both anchors hold.\n"
            "- The **φ ↔ η_I curves** solve the closed-form φ that closes the "
            "relevant anchor (USGS for break/smelt/refine, install for mfg).\n"
            "- **Monte Carlo** is parametric only. It does not capture the k/τ "
            "or φ ambiguities — those are the φ-curves above."
        )
